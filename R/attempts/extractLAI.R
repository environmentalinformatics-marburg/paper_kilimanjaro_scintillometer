...
fls_wht <- list.files("data/MCD15A2H.006/whittaker", 
                      pattern = "^MCD15A2H.*.tif$", full.names = TRUE)
fls_wht <- fls_wht[grep("2014001", fls_wht):grep("2014361", fls_wht)]
dts_wht <- as.Date(extractDate(fls_wht, 11, 17)$inputLayerDates, "%Y%j")
...
## write to file
saveRDS(dat_prm, paste0(dir_old, "/data/modis_lai.rds"))
